function knowMore() {
  ...
  alert('В Ирландии никогда не было кротов');

  if (confirm('Вы узнали больше?')) {
    knownMore = true;
    promo.classList.remove('red');
    promo.classList.remove('promo-gradient');
    promo.classList.add('green');
  } else {
    knownMore = false;
    promo.classList.remove('green');
    promo.classList.remove('promo-gradient');
    promo.classList.add('red');
  }

  if (knownMore === false) alert('Нажмите "Узнать больше"')
}
...
function checkName() {
  ...
  if (regex.test(name)) {
    ...
    document.getElementById("nameError").innerHTML = "Введите от 2 до 15 символов!";
    return false;
  }
}
...
function checkLName() {
  ...
  if (regex.test(lastName)) {
    ...
    document.getElementById("lnameSuccess").style.display = "block";
    return true;
  }
  else {
    document.getElementById("lnameSuccess").style.display = "none";
    document.getElementById("lnameError").style.color = "red";
    document.getElementById("lnameError").style.fontWeight = "bold";
    document.getElementById("lnameError").innerHTML = "Введите от 2 до 15 символов!";
    return false;
  }
}
...
function checkPhone() {
  ...
  if (regex.test(phone)) {
    ...
    document.getElementById("phoneError").style.fontWeight = "bold";
    document.getElementById("phoneError").innerHTML = "Формат номера 123-456-789!";
    return false;
  }
}
...
function checkEmail() {
  ...
  if (regex.test(email)) {
    ...
    document.getElementById("emailError").innerHTML = "Формат почты email@email.email!";
    return false;
  }
}
...
function checkAddress() {
  ...
  let regex = /^[a-zA-ZА-Яа-я\s]{10,100}$/;
  ...
  if (regex.test(address)) {
    ...
    document.getElementById("addressError").style.fontWeight = "bold";
    document.getElementById("addressError").innerHTML = "Введите от 10 до 100 символов!";
    return false;
  }
}
...
function checkCount() {
  ...
  if (regex.test(count)) {
    ...
    document.getElementById("countError").style.color = "red";
    document.getElementById("countError").style.fontWeight = "bold";
    document.getElementById("countError").innerHTML = "Введите количество от 1!";
    return false;
  }
}
...
function getSummaryData() {
  ...
  if (name != "" && lastName != "" && ((phone != "") && (checkPhone() === true)) && email != "" && address != "" && count != "") {
    document.getElementById("summary__info").style.color = "black";
    document.getElementById("summary__info").innerHTML = "Привет, " + name + " " + lastName + "!";
    document.getElementById("summary__info").innerHTML += "<p>Ваш заказ был успешно оформлен. Мы свяжемся с вами как можно скорее.</p>";
    document.getElementById("summary__info").innerHTML += "<p>Пожалуйста, проверьте данные заказа:</p>";
    document.getElementById("summary__info").innerHTML += "Номер телефона: " + phone + ".</br>";
    document.getElementById("summary__info").innerHTML += "Почтовый адрес: " + email + ".</br>";
    document.getElementById("summary__info").innerHTML += "Адрес доставки: " + address + ".</br>";
    document.getElementById("summary__info").innerHTML += "Количество чего-то крутого: " + count + ".</br>";
    document.getElementById("summary__info").innerHTML += "Способ оплаты: " + paymethod + ".</p>";
    document.getElementById("summary__info").innerHTML += "<p style='font-weight: bold;'>Спасибо, что Вы воспользовались нашими услугами! =)</p>";
  }
  else {
    document.getElementById("summary__info").style.color="red";
    document.getElementById("summary__info").innerHTML = "Пожалуйста, заполните все поля";
  }
}
